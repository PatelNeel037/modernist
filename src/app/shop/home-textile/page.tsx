...
export default function HomeTextilePage() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            // Filter by main category 'Home Textile'
            const homeTextileData = data.filter((p: any) => p.category === 'Home Textile' && p.status !== 'deleted' && p.status !== 'hidden');
            setProducts(homeTextileData);
            setLoading(false);
        };
        load();
    }, []);
        ...
        <main className="min-h-screen bg-white">
            ...
            <div className="relative pt-40 pb-20 text-center overflow-hidden mb-12 flex items-center justify-center min-h-[45vh]">
                <div className="absolute inset-0 z-0">
                    <img src="https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?q=80&w=2000&auto=format&fit=crop" alt="Home Textile" className="w-full h-full object-cover scale-105" />
                ...
                <div className="relative z-10 container mx-auto px-6 inline-block mt-10">
                    <div className="backdrop-blur-xl bg-white/10 border border-white/20 p-12 rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] max-w-3xl mx-auto">
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-playfair font-bold text-white mb-6 drop-shadow-lg">Home Textile</h1>
                        <p className="text-gray-100 max-w-2xl mx-auto text-lg md:text-xl drop-shadow font-medium tracking-wide leading-relaxed">Premium textiles to elevate your living spaces. Discover comfort and style for your home.</p>
                    </div>
                </div>
            </div>
            ...
            <section className="container mx-auto px-6 py-12">
                <div className="flex flex-col lg:flex-row gap-12 items-start">
                    ...
                    <aside className="hidden lg:block w-64 sticky top-24 shrink-0 space-y-8">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold border-b border-gray-200 pb-2 mb-4">Material</h3>
                            <div className="space-y-2">
                                {['Cotton', 'Linen', 'Silk', 'Wool', 'Bamboo'].map(fabric => (
